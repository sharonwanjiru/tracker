...
export default class main extends app.app {
    ...
    async reply_msg(): Promise<void>{
        ...
        const msg:{text:string, subject:string, event?:string}| undefined = await this.get_message_text(pk);
        //
    ...
    async get_message_text(pk: number): Promise<{text:string, subject:string, event?:string}| undefined> {
        //
        ...
        this.msg= await server.exec(
            ...
            ["mutall_users"],
            "get_sql_data",
            [`SELECT text, subject FROM  msg WHERE msg.msg = ${pk}`]
        );
        //
