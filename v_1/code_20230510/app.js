...
function clicked(event) {
    ...
    c1.setAttribute("cx", m.x);
    //
    //Set the y coordinate of the circle.
    c1.setAttribute("cy", m.y);
    //Set the line's xl and y1 co-ordinate.
    line.setAttribute("xl", m.x);
    line.setAttribute("yl", m.y);
}
// 
// Add an event listener to the svg.
svg.addEventListener("click", clicked)
    //
    //Get the svg layout on page load
function oMousePosSVG(e) {
    var p = svg.createSVGPoint();
    p.x = e.pageX;
    p.y = e.pageY;
    var cod = svg.getScreenCTM().inverse();
    var k = p.matrixTransform(cod);
    return k;
}
