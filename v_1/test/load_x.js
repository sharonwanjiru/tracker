...
export class load_tables extends outlook.terminal {
    ...
    async check() {
        //
        //Get the table args.
        const layouts = await this.get_layouts();
        //
    ...
    async get_layouts() {
        ...
        const tnames = ["client", "msg", "wmeter", "room", "emeter", "eaccount", "period", "service", "business"];
        //
        ...
        const table_layouts = await Promise.all(tnames.map(async (tname) => {
            ...
            const table_layout = await this.get_table_layout(cnames, tname);
            //
    ...
    async get_table_layout(cnames, tname) {
        //
        ...
        const dest = cnames.map(cname => [this.dest_dbname, tname, [tname], cname, [this.table, tname, cname]]);
        //
    ...
    async get_column_names(tname) {
        ...
        const column_names = fuel_cnames.map(cname => String(cname.COLUMN_NAME));
        //
