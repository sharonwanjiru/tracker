...
export async function display_md() {
    //
...
async function get_minute() {
    ...
    const rows = await server.exec('database', ['tracker_mogaka'], 'get_sql_data', [sql]);
    const minutes = rows.map(row => ({ surname: row.surname, name: row.name, problem: row.problem, plan: row.plan, outcome: row.outcome }));
    //
    //Return the minutes
    return minutes;
}
function create_markdown(minutes) {
    let md = '';
...
function create_filename(minutes) {
    //
...
async function save_file(md, filename) {
    //
    // get the directory
    ...
    await server.exec("path", [path, true], "put_file_contents", [md]);
