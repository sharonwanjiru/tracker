...
interface Iimagery {
  ...
  //
  //What do do if the decstnation is not empty
  action:'replace'|'skip'|'report';
  //
...
export class mashamba extends view.page {
  ...
  private async copy_from_source_to_destination(data_to_use:Iimagery):Promise<void>{
    //
    // 1. Get the  source 
    const source = <string> data_to_use.content;
    //
    ...
    if (!exists_source) data_to_use.action='report';    
    //
    ...
    //
    // 7.If it exists
      if (exists) data_to_use.action='report';     
  }
  //
